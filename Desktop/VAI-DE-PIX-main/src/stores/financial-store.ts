...
// Função removida - transações agora vêm da API
// Retorna array vazio - dados serão carregados da API
      ...
      updateTransaction: (id, updates) => set((state) => {
        ...
        let updatedAccounts = state.accounts.map(account => {
          if (account.id === oldTransaction.account) {
            if (oldTransaction.type === 'income') {
              return { ...account, balance: account.balance - Math.abs(oldTransaction.amount) };
            } else { // expense
              return { ...account, balance: account.balance + Math.abs(oldTransaction.amount) };
            }
          }
        ...
        updatedAccounts = updatedAccounts.map(account => {
          if (account.id === newAccountId) {
            if (newType === 'income') {
              return { ...account, balance: account.balance + Math.abs(newAmount) };
            } else { // expense
              return { ...account, balance: account.balance - Math.abs(newAmount) };
            }
          }
