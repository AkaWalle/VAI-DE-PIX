...
import { Plus, Wallet, ArrowLeftRight, Target, TrendingUp, Trash2 } from 'lucide-react';

export default function Envelopes() {
  const { envelopes, updateEnvelope, deleteEnvelope } = useFinancialStore();
  const { toast } = useToast();

  const handleDeleteEnvelope = (envelopeId: string, envelopeName: string) => {
  ...
  const totalTarget = envelopes.reduce((sum, env) => sum + (env.targetAmount || 0), 0);
        ...
        <Card className="bg-success/5 border-success/20">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {totalTarget > 0 ? ((totalBalance / totalTarget) * 100).toFixed(1) : 0}%
            </div>
            <p className="text-xs text-muted-foreground">
              Das metas atingidas
            </p>
      ...
      {envelopes.length === 0 ? (
        <Card className="bg-gradient-card shadow-card-custom">
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <h3 className="text-xl font-semibold mb-2">Nenhuma caixinha criada</h3>
            <p className="text-muted-foreground text-center mb-6 max-w-sm">
              Crie caixinhas para organizar seu orçamento por categoria ou objetivo
            </p>
            <EnvelopeForm 
              trigger={
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {envelopes.map((envelope) => {
            const progressPercentage = envelope.targetAmount 
              ...
              : 0;
            const isOverTarget = envelope.targetAmount && envelope.balance > envelope.targetAmount;

            return (
              <Card 
                key={envelope.id} 
                className="bg-gradient-card shadow-card-custom transition-all hover:shadow-financial"
                style={{ borderTopColor: envelope.color, borderTopWidth: '4px' }}
              >
                ...
                <CardContent className="space-y-4">
                  ...
                  {envelope.targetAmount && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className={`font-medium ${isOverTarget ? 'text-warning' : ''}`}>
                      ...
                      {isOverTarget && (
                        <div className="text-xs text-warning flex items-center gap-1">
                          ...
                          Acima da meta em {formatCurrency(envelope.balance - envelope.targetAmount)}
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="flex justify-center">
                    {envelope.targetAmount ? (
                      <Badge 
                        ...
                        className="text-xs"
                      >
                        {isOverTarget ? 'Acima da Meta' :
                         progressPercentage >= 100 ? 'Meta Atingida' :
                         progressPercentage >= 75 ? 'Quase Lá' :
                         progressPercentage >= 50 ? 'No Caminho' : 'Precisa de Mais'}
                      </Badge>
                    ) : (
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    <ConfirmDialog
                      trigger={
                        <ActionButton variant="outline" size="sm" className="px-2">
                      ...
                      confirmText="Remover"
                      onConfirm={() => handleDeleteEnvelope(envelope.id, envelope.name)}
                    />
                  </div>
                </CardContent>
              </Card>
        ...
        <Card className="bg-gradient-card shadow-card-custom">
          ...
          <CardContent>
            ...
            <div className="mt-4 flex gap-2">
              ...
              />
              <ActionButton icon={ArrowLeftRight}>
                Transferir
              </ActionButton>
            </div>
          </CardContent>
