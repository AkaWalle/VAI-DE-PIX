...
export default function Trends() {
  ...
  const incomeTrend = calculateTrend(last6MonthsData.map(d => ({ income: d.income, expense: 0 })));
  const expenseTrend = calculateTrend(last6MonthsData.map(d => ({ income: 0, expense: d.expense })));
  ...
  const categoryTrends = useMemo(() => {
    ...
    return categories.map(category => {
      ...
      const lastTotal = lastMonthTransactions.reduce((sum, t) => sum + Math.abs(t.amount), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="bg-gradient-card shadow-card-custom">
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <div className={`text-2xl font-bold ${incomeTrend.direction === 'up' ? 'text-income' : incomeTrend.direction === 'down' ? 'text-expense' : 'text-muted-foreground'}`}>
                {incomeTrend.trend.toFixed(1)}%
              </div>
              {incomeTrend.direction === 'up' && <TrendingUp className="h-5 w-5 text-income" />}
              {incomeTrend.direction === 'down' && <TrendingDown className="h-5 w-5 text-expense" />}
              {incomeTrend.direction === 'neutral' && <Target className="h-5 w-5 text-muted-foreground" />}
            </div>
            <p className="text-xs text-muted-foreground">
              {incomeTrend.direction === 'up' ? 'Crescimento' : incomeTrend.direction === 'down' ? 'Declínio' : 'Estável'} nos últimos 6 meses
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-card shadow-card-custom">
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <div className={`text-2xl font-bold ${expenseTrend.direction === 'up' ? 'text-expense' : expenseTrend.direction === 'down' ? 'text-income' : 'text-muted-foreground'}`}>
                {expenseTrend.trend.toFixed(1)}%
              </div>
              {expenseTrend.direction === 'up' && <TrendingUp className="h-5 w-5 text-expense" />}
              {expenseTrend.direction === 'down' && <TrendingDown className="h-5 w-5 text-income" />}
              {expenseTrend.direction === 'neutral' && <Target className="h-5 w-5 text-muted-foreground" />}
            </div>
            <p className="text-xs text-muted-foreground">
              {expenseTrend.direction === 'up' ? 'Aumento' : expenseTrend.direction === 'down' ? 'Redução' : 'Estável'} nos gastos
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-card shadow-card-custom">
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <div className={`text-2xl font-bold ${balanceTrend.direction === 'up' ? 'text-income' : balanceTrend.direction === 'down' ? 'text-expense' : 'text-muted-foreground'}`}>
                {balanceTrend.trend.toFixed(1)}%
              </div>
              {balanceTrend.direction === 'up' && <TrendingUp className="h-5 w-5 text-income" />}
              {balanceTrend.direction === 'down' && <TrendingDown className="h-5 w-5 text-expense" />}
              {balanceTrend.direction === 'neutral' && <Target className="h-5 w-5 text-muted-foreground" />}
            </div>
            <p className="text-xs text-muted-foreground">
              ...
              <div key={trend.category} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    {trend.direction === 'stable' && <Target className="h-3 w-3 mr-1" />}
                    {Math.abs(trend.change).toFixed(0)}%
                  </Badge>
                </div>
                <div className="text-right">
                  <div className="font-semibold">{formatCurrency(trend.currentTotal)}</div>
                  <div className="text-xs text-muted-foreground">
                    {trend.change > 0 ? '+' : ''}{formatCurrency(trend.currentTotal - trend.lastTotal)} vs mês anterior
                  </div>
                </div>
              </div>
            ))}
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="text-center p-4 rounded-lg bg-income/10 border border-income/20">
              <div className="text-sm text-muted-foreground mb-1">Receitas Previstas</div>
              <div className="text-2xl font-bold text-income">{formatCurrency(predictions.income)}</div>
            </div>
            <div className="text-center p-4 rounded-lg bg-expense/10 border border-expense/20">
              <div className="text-sm text-muted-foreground mb-1">Despesas Previstas</div>
              <div className="text-2xl font-bold text-expense">{formatCurrency(predictions.expense)}</div>
            </div>
            <div className={`text-center p-4 rounded-lg ${predictions.balance >= 0 ? 'bg-income/10 border-income/20' : 'bg-expense/10 border-expense/20'}`}>
              <div className="text-sm text-muted-foreground mb-1">Saldo Previsto</div>
              <div className={`text-2xl font-bold ${predictions.balance >= 0 ? 'text-income' : 'text-expense'}`}>
