...
class TestInputSanitizer:
    ...
    def test_sanitize_name_removes_html(self):
        """Testa que tags HTML são removidas (conteúdo entre tags é sanitizado)"""
        assert sanitize_name("<script>alert('xss')</script>Teste") == "alert(xss)Teste"
        assert sanitize_name("<b>Nome</b>") == "Nome"
    
    def test_sanitize_name_removes_sql_injection(self):
        """Testa que caracteres perigosos de SQL injection são removidos"""
